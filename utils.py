...
def get_attention(sess, model, params, dataset, domain_flag=True, eval_batch_size=100):
    ...
    while True:
        ...
        # asp_attention, opn_attention, ts_attention = sess.run([model.asp_attentions, model.opn_attentions, model.ts_attentions], feed_dict=feed_dict)
        # asp_attentions.extend(asp_attention.tolist())
        # opn_attentions.extend(opn_attention.tolist())
        # ts_attentions.extend(ts_attention.tolist())

        # asp_attention, opn_attention, dom_attention = sess.run([model.asp_attentions, model.opn_attentions, model.dom_attentions], feed_dict=feed_dict)
        # asp_attentions.extend(asp_attention.tolist())
        # opn_attentions.extend(opn_attention.tolist())
        # dom_attentions.extend(dom_attention.tolist())

        asp_attention, opn_attention = sess.run([model.asp_attentions, model.opn_attentions], feed_dict=feed_dict)
        asp_attentions.extend(asp_attention.tolist())
        opn_attentions.extend(opn_attention.tolist())

        # asp_attention, opn_attention, asp_pred, asp_pred_old, asp_d_pred, conf = sess.run([model.asp_attentions, model.opn_attentions, model.asp_pred, model.asp_pred_old, model.asp_pred_d, model.ote_conf], feed_dict=feed_dict)
        # asp_attentions.extend(asp_attention.tolist())
        # opn_attentions.extend(opn_attention.tolist())
        #
        # asp_predictions.extend(asp_pred.tolist())
        # asp_old_predictions.extend(asp_pred_old.tolist())
        # asp_d_predictions.extend(asp_d_pred.tolist())
        # confs.append(conf)
        # print(conf.shape)

        # opn_attention = sess.run(model.opn_attentions, feed_dict=feed_dict)
        # opn_attentions.extend(opn_attention.tolist())

    # return asp_attentions, opn_attentions, dom_attentions
    # return asp_attentions, opn_attentions, ts_attentions
    return asp_attentions, opn_attentions
    # return asp_attentions, opn_attentions, [asp_predictions, asp_old_predictions, asp_d_predictions, confs]
    # return opn_attentions

def visualization(sess, model, params, dataset, name, domain_flag=None, output_results=None):

    _, _, pred_ote, pred_ts = eval_metric(sess, model, params, dataset, domain_flag=domain_flag)
    # asp_attentions, opn_attentions, dom_attentions = get_attention(sess, model, params, dataset, domain_flag=domain_flag)
    asp_attentions, opn_attentions = get_attention(sess, model, params, dataset, domain_flag=domain_flag)
    # asp_attentions, opn_attentions, [asp_predictions, asp_old_predictions, asp_d_predictions, confs] = get_attention(sess, model, params, dataset, domain_flag=domain_flag)
    print(len(asp_attentions))
    print(len(opn_attentions))
    # print(len(dom_attentions))

    n_sample = len(dataset['lm_labels'])
    gold_ote = dataset['ote_tags']
    gold_ts  = dataset['ts_tags']
    output_lines = ['Source domain: %s  ' % params.source_domain, 'Target domain: %s\n' % params.target_domain, 'Parameter settings: \n']
    ...
    if output_results != None:
        output_lines.append(output_results)

    output_lines.append("==============================================\n\n")
    for i in range(n_sample):
        ote_seq = pred_ote[i]
        ts_seq = pred_ts[i]
        asp_attention = asp_attentions[i]
        opn_attention = opn_attentions[i]
        # dom_attention = dom_attentions[i]
        w_seq = dataset['words'][i]
        length = dataset['length'][i]
        ote_seq_gold = gold_ote[i]
        ts_seq_gold = gold_ts[i]

        # asp_prediction = asp_predictions[i]
        # asp_old_prediction = asp_old_predictions[i]
        # asp_d_prediction = asp_d_predictions[i]
        # conf = confs[i]

        output_lines.append('Review#%d\n' % i)
        ...
        for j in range(length):
            ...
            asp_alpha = asp_attention[j]
            opn_alpha = opn_attention[j]
            # dom_alpha = dom_attention[j]

            # output_lines.append('%20s\t%s\t%s\t%.4f\t%.4f\t%s\t%s\t%.4f\n' % (word, ote_tag, ote_tag_gold, asp_alpha, dom_alpha, ts_tag, ts_tag_gold, opn_alpha))
            output_lines.append('%20s\t%s\t%s\t%.4f\t%s\t%s\t%.4f\n' % (word, ote_tag, ote_tag_gold, asp_alpha, ts_tag, ts_tag_gold, opn_alpha))

            # output_lines.append('%20s\t%s\t%s\t%.4f\t%s\t%s\t%.4f' % (word, ote_tag, ote_tag_gold, asp_alpha, ts_tag, ts_tag_gold, opn_alpha))
            # for alpha in asp_prediction[j]:
            #     output_lines.append('\t%.4f' % alpha)
            # output_lines.append('|||')
            # for alpha in asp_old_prediction[j]:
            #     output_lines.append('\t%.4f' % alpha)
            # output_lines.append('|||')
            # for alpha in asp_d_prediction[j]:
            #     output_lines.append('\t%.4f' % alpha)
            # output_lines.append('|||')
            # # output_lines.append('\t%.4f' %  conf)
            # output_lines.append('\n')

        # use empty lines as the separator
        output_lines.append('\n')

    output_path = './work/visualization/'
    if not os.path.exists(output_path):
        os.mkdir(output_path)

    model_path = output_path + "%s/" % params.model_name
    if not os.path.exists(model_path):
        os.mkdir(model_path)

    file_path = model_path+'%s_%s-%s_round%d.txt' % (params.source_domain, params.target_domain, name, params.round)
    with open(file_path, 'w+') as fp:
        fp.writelines(output_lines)
    fp.close()

# def visualization2(sess, model, params, dataset, name, domain_flag=None):
#
#     _, pred_ts = eval_metric(sess, model, params, dataset, domain_flag=domain_flag)
#     asp_attentions, opn_attentions = get_attention(sess, model, params, dataset, domain_flag=domain_flag)
#     print(len(asp_attentions))
#     print(len(opn_attentions))
#
...
#             opn_alpha = opn_attention[j]
#
#             output_lines.append('%20s\t%.4f\t%s\t%s\t%.4f\n' % (word, asp_alpha, ts_tag, ts_tag_gold, opn_alpha))
#         # use empty lines as the separator
#         output_lines.append('\n')
#
#     output_path = './work/visualization/'
#     if not os.path.exists(output_path):
#         os.mkdir(output_path)
#     model_path = output_path+'%s_%s_%s.txt' % (name, params.source_domain, params.target_domain)
#     with open(model_path, 'w+') as fp:
#         fp.writelines(output_lines)

def visualization3(sess, model, params, dataset, name, domain_flag=None):

    _, _, pred_ote, pred_ts = eval_metric(sess, model, params, dataset, domain_flag=domain_flag)
    asp_attentions, opn_attentions, ts_attentions = get_attention(sess, model, params, dataset, domain_flag=domain_flag)
    print(len(asp_attentions))
    print(len(opn_attentions))
    print(len(ts_attentions))
    ...
    for i in range(n_sample):
        ...
        ote_seq_gold = gold_ote[i]
        ts_seq_gold = gold_ts[i]
        idxs = [idx for idx, tag in enumerate(ote_seq_gold) if tag != 'O']

        # print(w_seq)
        # print(length)
        # print(ts_seq_gold)
        # for a in np.array(opn_attentions[i]):
        #     print(a)
        # exit(0)
        ts_attention = np.array(ts_attentions[i])[idxs,:]
        assert len(ote_seq) == len(ts_seq) == len(w_seq)
        for j in range(length):
            word = w_seq[j]
            ote_tag = ote_seq[j]
            ...
            ts_tag = ts_seq[j]
            ts_tag_gold = ts_seq_gold[j]
            ts_alpha = ts_attention[:,j]
            asp_alpha = asp_attention[j]
            opn_alpha = opn_attention[j]

            output_lines.append('%20s\t%s\t%s\t%.4f\t%s\t%s\t%.4f' % (word, ote_tag, ote_tag_gold, asp_alpha, ts_tag, ts_tag_gold, opn_alpha))

            for alpha in ts_alpha:
                output_lines.append('\t%.4f' % alpha)
            output_lines.append('\n')
